...
static int test_dma(char *devicename, uint64_t addr, uint64_t size, uint64_t offset, uint64_t count, char *filename);
...
static uint64_t getopt_integer(char *optarg)
{
  int rc;
  uint64_t value;
  rc = sscanf(optarg, "0x%llx", &value);
  if (rc <= 0)
    rc = sscanf(optarg, "%lld", &value);
  printf("sscanf() = %d, value = 0x%llx\n", rc, (uint64_t)value);
  return value;
}
...
int main(int argc, char* argv[])
{
  ...
  printf("device = %s, address = 0x%016x, size = 0x%016x, offset = 0x%016x, count = %u\n", device, address, size, offset, count);
...
static int test_dma(char *devicename, uint64_t addr, uint64_t size, uint64_t offset, uint64_t count, char *filename)
{
  ...
  /* fill the buffer with data from file? */
    ...
    off64_t off = lseek(fpga_fd, addr, SEEK_SET);
  while (count--) {
